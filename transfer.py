...
def revoke_all_permissions(service, drive_item):
    try:
        for permission in (
            service.permissions().list(fileId=drive_item["id"]).execute()["items"]
        ):
            if permission["id"] in [
                owner["permissionId"] for owner in drive_item["owners"]
            ]:
                print("can't remove owner")
                continue
            service.permissions().delete(
                fileId=drive_item["id"], permissionId=permission["id"]
            ).execute()
    except googleapiclient.errors.HttpError as e:
        if e.resp.status != 404:
            print("An error occurred deleting permissions: {}".format(e))
            return
    print(f"all permissions for item {drive_item['title']} have been removed.")
...
def grant_ownership(service, drive_item, prefix, permission_id, show_already_owned):
    ...
    for owner in drive_item["owners"]:
        if owner["permissionId"] == permission_id:
            if FLAG_REVOKE_ALL_ACCESS:
                revoke_all_permissions(service, drive_item)
...
if __name__ == "__main__":
    ...
    show_already_owned = False if sys.argv[3] == "false" else True
    FLAG_REVOKE_ALL_ACCESS = True if sys.argv[4] == "true" else False
    print(f"running with FLAG_REVOKE_ALL_ACCESS: {FLAG_REVOKE_ALL_ACCESS}")
